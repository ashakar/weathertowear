...
def get_index():
    ...
    if not (alternates == ""):
        alternates = f"If you are going out for a while, igt also wouldn't be the worst idea to have {alternates} or similar wear on hand because the weather today is prone to high variability."
...
def processWeather(data):
    ...
    if "sleet" in condition or "snow" in condition or "ice" in condition:
        ...
        if "ice" in condition:
            analysis+= f"Note: It feels like {temperature} F, but due to the ice, you will be a lot more comofortable if you dress as if it were {int(temperature - 3)} F."
            temperature-=3
